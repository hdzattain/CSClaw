...
interface SyncProxyOptions {
   ...
   * currently disabled in CSClaw settings.
   */
  preserveExistingWhenDisabled?: boolean;
}

/**
 * Sync CSClaw global proxy settings into OpenClaw channel config where the
...
export async function syncProxyConfigToOpenClaw(
  ...
  return withConfigLock(async () => {
    ...
    if (!settings.proxyEnabled && preserveExistingWhenDisabled && currentProxy) {
      logger.info('Skipped Telegram proxy sync because CSClaw proxy is disabled and preserve mode is enabled');
      return;
    }

...
function getElectronApp() {
  ...
  const fallbackUserData = process.env.CSCLAW_USER_DATA_DIR?.trim() || join(homedir(), '.csclaw');
...
/**
 * Get CSClaw config directory
 */
export function getCSClawConfigDir(): string {
  return join(homedir(), '.csclaw');
}

/**
 * Get CSClaw logs directory
 */

...
function resolveManifestPath(): string {
  if (app.isPackaged) {
    return join(process.resourcesPath, 'csclaw-extensions.json');
  }
  return join(app.getAppPath(), 'csclaw-extensions.json');
}
...
export async function loadExtensionsFromManifest(): Promise<void> {
  ...
  if (existsSync(manifestPath)) {
    ...
    logger.debug('[extensions] No csclaw-extensions.json found, loading all builtin extensions');
  }

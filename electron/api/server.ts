...
export function startHostApiServer(ctx: HostApiContext, port = getPort('CSCLAW_HOST_API')): Server {
  ...
  server.on('error', (error: NodeJS.ErrnoException) => {
    if (error.code === 'EACCES' || error.code === 'EADDRINUSE') {
      logger.error(
        ...
        `Set CSCLAW_PORT_CSCLAW_HOST_API env var to override the default port.`,
      );
    } else {

...
import { ensureCSClawContext } from '../../utils/openclaw-workspace';
    ...
    if (pid) {
      ...
      try {
        if (process.platform === 'darwin' || process.platform === 'linux') {
          // MUST use -sTCP:LISTEN. Otherwise lsof returns the client process (CSClaw itself) 
...
export async function handleAgentRoutes(
  ...
  if (url.pathname === '/api/agents' && req.method === 'POST') {
    try {
      ...
      // Ensure newly provisioned workspaces get CSClaw context merge/cleanup
      // even when gateway status events do not fire (e.g. in-process reload).
      void ensureCSClawContext({ waitForAllConfiguredWorkspaces: true }).catch((err) => {
        console.warn('[agents] Failed to ensure CSClaw context after agent creation:', err);
      });

...
Use this spec when CSClaw shows the Gateway as starting/running but UI data does not refresh, Dreams cannot load, or Gateway RPC calls time out after a restart.

CSClaw should prefer OpenClaw-native signals over stderr string matching:
...
- `gateway.ready`, `health`, and `presence` events should update CSClaw's main-process capability cache.
...
- **Handshake ready** only means CSClaw connected to the Gateway socket.
...
2. Read recent CSClaw logs:

```bash
tail -n 160 "$HOME/Library/Application Support/csclaw/logs/csclaw-$(date +%F).log"
```
...
```bash
pnpm exec openclaw gateway call system-presence >/tmp/csclaw-system-presence.json
pnpm exec openclaw gateway call health --params '{"probe":false}' >/tmp/csclaw-health.json
pnpm exec openclaw gateway call status >/tmp/csclaw-status.json
pnpm exec openclaw gateway call channels.status --params '{"probe":false}' >/tmp/csclaw-channels-status.json
pnpm exec openclaw gateway call doctor.memory.status >/tmp/csclaw-memory-status.json
pnpm exec openclaw gateway call doctor.memory.dreamDiary >/tmp/csclaw-dream-diary.json
```
...
```bash
sample <gateway-pid> 3 -mayDie >/tmp/csclaw-gateway.sample.txt
```
...
- Where the artifact will be written, usually `/tmp/csclaw-gateway.sample.txt`.
- What will be inspected and what will not be shared verbatim.

Do not proceed without explicit user agreement when:

- Sampling a process that is not clearly the CSClaw-owned Gateway child.
...
- Whether the finding points to CSClaw-owned prelaunch cleanup, OpenClaw runtime startup cost, active user work, or inconclusive data.
...
- Explain to users that restart cost is dominated by active Gateway work, not by CSClaw UI rendering.
...
- Logs no longer repeat stale runtime cache or escaped managed-skill symlink warnings for entries CSClaw can safely clean.
...
- Separate CSClaw-owned cleanup issues from OpenClaw runtime initialization cost.
